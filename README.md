...
2. **데이터 소스 (종목·용도별)**
   ...
   | 전고점 일봉 (5년, 최고가) | **KRX(pykrx)** → 야후 (폴백) | 야후 파이낸스 |
...
3. **전고점 대비 하락률 (Drawdown Analysis)**: 
   - 과거 5년 치 일봉의 **고가 중 최고값**을 전고점으로 사용합니다. HTS 지수차트에 표시되는 `최고 9,385.59` 와 동일한 기준입니다.
   - 현재 지수가 이 전고점 대비 얼마나 하락해 있는지(예: `-27.46%`)를 계산하여, 현재 시장이 상승장인지 조정장인지 수치로 직관적으로 보여줍니다.
   - 신고가를 갱신 중이면 `0%`로 표기합니다 (낙폭이 양수가 되지 않음).
   - ⚠️ 과거에는 '앞뒤 15일 윈도우의 직전 스윙 하이'를 전고점으로 삼았으나, 고점에서 내려온 뒤 소폭 반등하면 그 낮은 봉우리를 전고점으로 잡아 낙폭이 실제(-27%)보다 크게 축소(-10%)되는 문제가 있어 최고가 기준으로 변경했습니다.
4. **AI 매크로 한줄평 생성**:
...
| :--- | :--- | :--- | :--- | :--- |
| **지수 급락 경보 / 분할매수** | **KOSPI / KOSDAQ** | **09:00~15:30 KST** | **15분** | **전고점 대비 낙폭 단계 진입** |
...
전고점(5년 일봉 최고가) 대비 낙폭이 아래 단계에 도달하면 `[N차 매수 추천]` 알림이 발송됩니다.
지수는 `INDEX_CRASH_RULES`, ETF는 `LEVERAGE_ETF_RULES`에 정의되어 있으며 계산은 `calc_buy_tier()`가 공용으로 담당합니다.

| 종목 | 1차 진입 | 추가 매수 간격 | 예시 |
| :--- | :---: | :---: | :--- |
| KOSPI | -15% | **5%마다** | -15% 1차, -20% 2차, -25% 3차 … |
| KOSDAQ | -20% | **5%마다** | -20% 1차, -25% 2차, -30% 3차 … |
| TQQQ | -10% | **1%마다** | -10% 1차, -11% 2차, -12% 3차 … |
| QLD | -10% | **5%마다** | -10% 1차, -15% 2차, -20% 3차 … |

> 코스닥은 코스피보다 낙폭이 통상 1.3~1.5배 크므로 진입 시작점을 5%p 높게 잡았습니다.

- 알림은 **더 깊은 단계로 내려갈 때만** 발송됩니다. 같은 구간에 머무르거나 낙폭이 얕아지는 동안은 조용합니다.
- **전고점을 회복(낙폭 0%)하면** 상태가 초기화되어, 다음 하락에 1차부터 다시 알림이 울립니다.
- 발송 이력은 `.cache/alert_state.json`에 저장되어 **봇을 재시작해도 이어집니다.** 재시작 때문에 같은 단계 알림이 다시 나가지 않습니다.
  - 임시 파일에 쓴 뒤 `os.replace`로 원자적 교체하므로 저장 도중 종료돼도 파일이 깨지지 않습니다.
  - 파일이 없거나 깨져 있으면 경고만 남기고 초기값으로 시작합니다. (깨진 파일 때문에 알림이 멈추는 것보다 안전)
  - 알림을 처음부터 다시 받고 싶으면 이 파일을 지우면 됩니다.
- 개장 판정은 KST 고정 시각이 아닌 ET 기준이므로 **서머타임(EDT/EST)에 자동 대응**합니다. (KST 기준 여름 22:30~05:00 / 겨울 23:30~06:00)
...
| 이미지 발송 테스트 | `test_telegram_photo.py` — 차단 채팅방(403) 처리, file_id 재사용, 부분 실패 오탐 방지 |
...
graph TD
    ...
    subgraph "15:45 KST 지수 분석 (Index Closing)"
        I1([국내외 주요지수 · ETF 수집]) --> I2[전고점(최고가) 대비 하락률 연산]
...
*   `test_telegram_photo.py`: 차단된 채팅방 처리, file_id 재사용, 부분 실패 시 오탐 알림 방지 검증. **CI 자동 실행.**
